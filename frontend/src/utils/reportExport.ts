...
function safeName(s: string) {
  return s.replace(/[^a-z0-9]+/gi, '_').replace(/^_+|_+$/g, '') || 'WalletTrails'
}

/** ── CSV export ───────────────────────────────────────────────── */
export function downloadLedgerCSV(rows: LedgerRow[], meta: ReportMeta) {
  const lines: string[] = []
  lines.push(`WalletTrails — Financial Report`)
  ...
  triggerDownload(blob, `${safeName('WalletTrails_' + meta.username + '_' + meta.monthLabel)}.csv`)
}
...
export async function downloadReportPDF(rows: LedgerRow[], meta: ReportMeta) {
  ...
  doc.setFontSize(22)
  doc.text('WalletTrails', textX, 44)
  ...
  autoTable(doc, {
    ...
    didDrawPage: () => {
      ...
      doc.setFontSize(8)
      doc.setTextColor(...MUTED)
      doc.text('Generated by WalletTrails — follow every rupee', margin, h - 18)
  ...
  doc.save(`${safeName('WalletTrails_' + meta.username + '_' + meta.monthLabel)}.pdf`)
}
...
export function downloadHouseholdReportCSV(rows: HouseholdReportRow[], meta: HouseholdReportMeta) {
  const lines: string[] = []
  lines.push('WalletTrails — Household Report')
  ...
  triggerDownload(blob, `WalletTrails_Household_${label}.csv`)
}
